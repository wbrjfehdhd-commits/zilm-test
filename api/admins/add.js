...
const ALLOWED_ROLES = ['admin', 'moderator', 'wiki-editor']; // owner is fixed via env vars, not creatable here
...
module.exports = async (req, res) => {
  ...
  if (!username || !password || !ALLOWED_ROLES.includes(role)) {
    res.status(400).json({ error: 'Username, password, and a valid role (admin/moderator/wiki-editor) are required.' });
    return;
  }
